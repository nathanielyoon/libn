...
const UPPER = IV.subarray(160, 176), LOWER = UPPER.filter((_, z) => z & 1 ^ 1);
const md = (
  base: Uint32Array,
...
/** Hashes with SHA-512. */
export const sha512: ReturnType<typeof md> = md(UPPER, (use, data, at, to) => {
  let a, b, c, d, z = 0;
  ...
  do a = (n >>> 9 | m << 23) ^ (m >>> 14 | n << 18) ^ (m >>> 18 | n << 14),
    ...
    (m = v + k + (n / 0x100000000 | 0) | 0, k = i, i = g, g = e, l = j, j = h),
    (h = f >>> 0, f = (b >>> 0) + (d >>> 0) + (u >>> 0)),
...
/** Hashes with SHA-256. */
export const sha256: ReturnType<typeof md> = md(LOWER, (use, data, at, to) => {
  let a, b, z = 0;
