...
Deno.test("spec", async ($) => {
  ...
  for (const $ of vectors.default.uncase) {
...
describe("fuzzy", () => {
  it("includes() follows built-in includes", () =>
  ...
  it("includes(string, substring) : true", () =>
  ...
  it("includes(shorter, longer) : false", () =>
    fc.assert(fc.property(
  ...
  it("includes(same-length, same-length) :: strict equality", () =>
    fc.assert(fc.property(
      fc.string({ unit: "grapheme" }).chain(($) =>
        fc.record({
          one: fc.constant($),
  ...
  it("distance() :: levenshtein", () =>
    fc.assert(fc.property(fcPair, ({ one, two }) => {
      expect(distance(one, two)).toBe(levenshtein(one, two));
    })));
  it("distance() falls inside levenshtein bounds", () =>
    fc.assert(fc.property(fcPair, ({ one, two }) => {
  ...
  it("distance() accounts for high code points", () =>
    fc.assert(fc.property(
      fc.record({
  ...
  it("distance() checks surrogate pairs together", () => {
...
describe("normalize", () => {
  it("unlone() replaces lone surrogates", () =>
    fc.assert(fc.property(
      fc.uint16Array().map(($) =>
  ...
  it("unlone() never changes string length", () =>
    fc.assert(fc.property(
      fc.stringMatching(
        /^(?:[\ud800-\udbff](?:[^\udc00-\udfff]|$)|(?:^|[^\ud800-\udbff])[\udc00-\udfff])*$/,
      ),
  ...
  it("uncode() replaces any unassignable code point", () =>
    fc.assert(fc.property(
      fc.stringMatching(
  ...
  it("uncode() replaces all lone surrogates", () => {
    for (let z = 0xd800; z <= 0xdfff; ++z) replaced(String.fromCharCode(z));
  });
  it("uncode() replaces all control codes", () => {
  ...
  it("unwide() collapses consecutive spaces", () =>
    fc.assert(fc.property(fc.string({ unit: "grapheme" }), ($) => {
      expect(unwide($)).toMatch(/^(?:\S|(\s|\r\n)(?!\1))*$/);
    })));
  it("unwide() matches any space", () =>
  ...
  it("unhtml() removes special html characters", () =>
    fc.assert(fc.property(fc.string(), ($) => {
      expect(unhtml($)).toMatch(/^(?:[^&"'<>]|&#\d\d;)*$/);
    })));
  it("unhtml() escapes with right codes", () =>
    fc.assert(fc.property(
  ...
  it("unrexp() makes literal", () =>
    fc.assert(fc.property(fc.string({ unit: "grapheme" }), ($) => {
      expect(RegExp(`^${unrexp($)}$`).exec($)?.[0]).toBe($);
    })));
  it("unrexp() matches all regex syntax characters", () => {
    for (const $ of "/^$\\*+?{}()[]|") expect(unrexp($)).toBe(`\\${$}`);
  });
  it("unrexp() matches all character-escapable characters", () => {
  ...
  it("unrexp() escapes weird characters", () => {
  ...
  it("unrexp() escapes the first character if alphanumeric", () => {
    for (let z = 0; z < 10; ++z) {
...
describe("case", () => {
  ...
  it("createRanges() matches source", async () => {
    expect(createRanges(
      await source().catch(($) => {
