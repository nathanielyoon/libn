...
const body = ($: Schema) => {
  ...
  let all = `if(`;
  if (isSchema.boolean($)) all += 'typeof U!=="boolean")';
  else if (isSchema.integer($) || isSchema.number($)) {
    all += `Number.is${isSchema.integer($) ? "SafeInteger" : "Finite"}(U)){`;
  ...
  return wrap`${all + (isArray($.type) ? " U===null||" : "")}${"type"}`;
};
