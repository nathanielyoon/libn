...
import { de_bin, de_u64, en_bin, en_u64 } from "@libn/base";
import { type Data, object, parser, string } from "@libn/json";
import { lift, no, ok, type Or, run, try_catch } from "@libn/result";
...
const PAYLOAD = object().properties({
...
/** PASETO payload. */
export type Payload = Data<typeof PAYLOAD>;
const parse = parser(PAYLOAD);
...
> =>
  ok(/^v4\.public\.(?<body>[-\w]{383,})\.(?<footer>[-\w]*)$/.exec($ ?? ""))
    ...
    .bind(run(function* ([_, body, footer]) {
      ...
      const e = yield* parse(d).fmap(($) => $, () => 400 as const);
