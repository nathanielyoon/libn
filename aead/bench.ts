...
fc_bench("encrypt", fc.tuple(fc_bin(32), fc_bin(24), fc_bin(100), fc_bin(10)), {
  ...
  stablelib: (key, iv, plaintext, ad) => {
    const output = new stablelib.XChaCha20Poly1305(key).seal(iv, plaintext, ad);
    return { ciphertext: output.subarray(0, -16), tag: output.subarray(-16) };
  },
}, 100);
fc_bench(
  "decrypt",
  fc.tuple(fc_bin(32), fc_bin(24), fc_bin(100), fc_bin(10)).map(
    ([key, iv, plaintext, ad]) =>
