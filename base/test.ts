...
test["b16 : binary"](fc.uint8Array(), ($) => {
  const string = enB16($);
  return assertMatch(string, B16), [deB16(string)];
});
...
test["b64.enB64 :: built-in toBase64"](
  fc.uint8Array(),
  ($) => [enB64($), $.toBase64()],
);
...
test["b64.deB64 : invalid string"](
  fc.stringMatching(RegExp(B64.source.replaceAll("[", "[^"))),
  ($) => [deB64($), deB64($).fill(0)],
);
test["b64.deB64 :: built-in fromBase64"](
  fc.uint8Array().map(enB64),
  ($) => [deB64($), Uint8Array.fromBase64($)],
);
...
test["u64.enU64 :: built-in toBase64"](
  fc.uint8Array(),
  ($) => [enU64($), $.toBase64({ alphabet: "base64url", omitPadding: true })],
);
test["u64.deU64 : valid string"]([
  ["", ""],
  ["SGVsbG8gd29ybGQh", "Hello world!"],
], ($) => [deU64($[0]), enUtf8($[1])]);
test["u64.deU64 : invalid string"](
  fc.stringMatching(RegExp(U64.source.replaceAll("[", "[^"))),
  ($) => [deU64($), deU64($).fill(0)],
);
test["u64.deU64 :: built-in fromBase64"](
  fc.uint8Array().map(enU64),
  ($) => [deU64($), Uint8Array.fromBase64($, { alphabet: "base64url" })],
);
