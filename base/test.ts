...
Deno.test("16", async ({ step }) => {
  ...
  await step("en_b16/de_b16 : arbitrary round-trip", () => {
    fc_check(
      fc.property(fc_binary(), ($) => assertEquals(de_b16(en_b16($)), $)),
    );
  });
});
Deno.test("32", async ({ step }) => {
  ...
  await step("en_b32/de_b32 : arbitrary round-trip", () => {
    fc_check(
      fc.property(fc_binary(), ($) => assertEquals(de_b32(en_b32($)), $)),
    );
  });
  await step("en_h32/de_h32 : arbitrary round-trip", () => {
    fc_check(
      fc.property(fc_binary(), ($) => assertEquals(de_h32(en_h32($)), $)),
    );
  });
  await step("en_z32/de_z32 : arbitrary round-trip", () => {
    fc_check(
      fc.property(fc_binary(), ($) => assertEquals(de_z32(en_z32($)), $)),
    );
  });
  await step("en_c32/de_c32 : arbitrary round-trip", () => {
    fc_check(
      fc.property(fc_binary(), ($) => assertEquals(de_c32(en_c32($)), $)),
    );
  });
});
Deno.test("64", async ({ step }) => {
  ...
  await step("en_b64/de_b64 : arbitrary round-trip", () => {
    fc_check(
      fc.property(fc_binary(), ($) => assertEquals(de_b64(en_b64($)), $)),
    );
  });
  await step("en_u64/de_u64 : arbitrary round-trip", () => {
    fc_check(
      fc.property(fc_binary(), ($) => assertEquals(de_u64(en_u64($)), $)),
    );
  });
});
