...
import { ce, Child, qa, qs } from "./mod.ts";
...
const { fc_html } = fc.letrec<{ fc_html: Element }>((tie) => ({
  fc_html: fc.tuple(
    fc_tag,
    fc_letter.map((name) => ($: Element) => $.className = name),
    fc.array(
      fc.oneof(fc.constantFrom(undefined, null), fc_str(), tie("fc_html")),
    ),
  ).map(([tag, assign, children]) => ce(tag, assign, ...children)),
}));
const fc_selector = fc.array(fc.oneof(fc_tag, fc_letter.map(($) => "." + $)), {
  minLength: 1,
}).map(($) => $.join(" "));
