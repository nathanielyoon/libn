...
// Since the first 32-byte value of the state buffer stores the current block's
// length, the message bytes (and thus these table indices) start at 1.
...
const b2s_add = ($: B2State) => ($[25] += $[0]) < Mod.U || ++$[26];
...
const b2b_add = ($: B2State) =>
  ($[49] += $[0]) < Mod.U || ++$[50] < Mod.U || ++$[51] < Mod.U || ++$[52];
// Each BLAKE2b word is split into two 32-bit halves, so the indices are really
...
const b2b_mix = ($: B2State, final: boolean) => {
  ...
  do a0 += e0 + $[PERMUTE_BOTH[z] - 1],
    a1 = a1 + e1 + $[PERMUTE_BOTH[z++]] + (a0 / Mod.U | 0) | 0,
    a0 >>>= 0,
    q = m0 ^ a0,
    i0 += m0 = (m1 ^ a1) >>> 0,
    m1 = q >>> 0,
    ...
    m1 ^= a1 = a1 + e1 + $[PERMUTE_BOTH[z++]] + (a0 / Mod.U | 0) | 0,
    a0 >>>= 0,
    q = m0 ^ a0,
    i0 += m0 = (q >>> 16 ^ m1 << 16) >>> 0,
    m1 = (m1 >>> 16 ^ q << 16) >>> 0,
    ...
    b1 = b1 + f1 + $[PERMUTE_BOTH[z++]] + (b0 / Mod.U | 0) | 0,
    b0 >>>= 0,
    q = n0 ^ b0,
    j0 += n0 = (n1 ^ b1) >>> 0,
    n1 = q >>> 0,
    ...
    f1 = (f1 >>> 24 ^ q << 8) >>> 0,
    b0 += f0 + $[PERMUTE_BOTH[z] - 1],
    n1 ^= b1 = b1 + f1 + $[PERMUTE_BOTH[z++]] + (b0 / Mod.U | 0) | 0,
    b0 >>>= 0,
    q = n0 ^ b0,
    j0 += n0 = (q >>> 16 ^ n1 << 16) >>> 0,
    n1 = (n1 >>> 16 ^ q << 16) >>> 0,
    ...
    c1 = c1 + g1 + $[PERMUTE_BOTH[z++]] + (c0 / Mod.U | 0) | 0,
    c0 >>>= 0,
    q = o0 ^ c0,
    k0 += o0 = (o1 ^ c1) >>> 0,
    o1 = q >>> 0,
    ...
    g1 = (g1 >>> 24 ^ q << 8) >>> 0,
    c0 += g0 + $[PERMUTE_BOTH[z] - 1],
    o1 ^= c1 = c1 + g1 + $[PERMUTE_BOTH[z++]] + (c0 / Mod.U | 0) | 0,
    c0 >>>= 0,
    q = o0 ^ c0,
    k0 += o0 = (q >>> 16 ^ o1 << 16) >>> 0,
    o1 = (o1 >>> 16 ^ q << 16) >>> 0,
    ...
    d1 = d1 + h1 + $[PERMUTE_BOTH[z++]] + (d0 / Mod.U | 0) | 0,
    d0 >>>= 0,
    q = p0 ^ d0,
    l0 += p0 = (p1 ^ d1) >>> 0,
    p1 = q >>> 0,
    ...
    h1 = (h1 >>> 24 ^ q << 8) >>> 0,
    d0 += h0 + $[PERMUTE_BOTH[z] - 1],
    p1 ^= d1 = d1 + h1 + $[PERMUTE_BOTH[z++]] + (d0 / Mod.U | 0) | 0,
    d0 >>>= 0,
    q = p0 ^ d0,
    l0 += p0 = (q >>> 16 ^ p1 << 16) >>> 0,
    p1 = (p1 >>> 16 ^ q << 16) >>> 0,
    ...
    a1 = a1 + f1 + $[PERMUTE_BOTH[z++]] + (a0 / Mod.U | 0) | 0,
    a0 >>>= 0,
    q = p0 ^ a0,
    k0 += p0 = (p1 ^ a1) >>> 0,
    p1 = q >>> 0,
    ...
    f1 = (f1 >>> 24 ^ q << 8) >>> 0,
    a0 += f0 + $[PERMUTE_BOTH[z] - 1],
    p1 ^= a1 = a1 + f1 + $[PERMUTE_BOTH[z++]] + (a0 / Mod.U | 0) | 0,
    a0 >>>= 0,
    q = p0 ^ a0,
    k0 += p0 = (q >>> 16 ^ p1 << 16) >>> 0,
    p1 = (p1 >>> 16 ^ q << 16) >>> 0,
    ...
    b1 = b1 + g1 + $[PERMUTE_BOTH[z++]] + (b0 / Mod.U | 0) | 0,
    b0 >>>= 0,
    q = m0 ^ b0,
    l0 += m0 = (m1 ^ b1) >>> 0,
    m1 = q >>> 0,
    ...
    g1 = (g1 >>> 24 ^ q << 8) >>> 0,
    b0 += g0 + $[PERMUTE_BOTH[z] - 1],
    m1 ^= b1 = b1 + g1 + $[PERMUTE_BOTH[z++]] + (b0 / Mod.U | 0) | 0,
    b0 >>>= 0,
    q = m0 ^ b0,
    l0 += m0 = (q >>> 16 ^ m1 << 16) >>> 0,
    m1 = (m1 >>> 16 ^ q << 16) >>> 0,
    ...
    c1 = c1 + h1 + $[PERMUTE_BOTH[z++]] + (c0 / Mod.U | 0) | 0,
    c0 >>>= 0,
    q = n0 ^ c0,
    i0 += n0 = (n1 ^ c1) >>> 0,
    n1 = q >>> 0,
    ...
    h1 = (h1 >>> 24 ^ q << 8) >>> 0,
    c0 += h0 + $[PERMUTE_BOTH[z] - 1],
    n1 ^= c1 = c1 + h1 + $[PERMUTE_BOTH[z++]] + (c0 / Mod.U | 0) | 0,
    c0 >>>= 0,
    q = n0 ^ c0,
    i0 += n0 = (q >>> 16 ^ n1 << 16) >>> 0,
    n1 = (n1 >>> 16 ^ q << 16) >>> 0,
    ...
    d1 = d1 + e1 + $[PERMUTE_BOTH[z++]] + (d0 / Mod.U | 0) | 0,
    d0 >>>= 0,
    q = o0 ^ d0,
    j0 += o0 = (o1 ^ d1) >>> 0,
    o1 = q >>> 0,
    ...
    e1 = (e1 >>> 24 ^ q << 8) >>> 0,
    d0 += e0 + $[PERMUTE_BOTH[z] - 1],
    o1 ^= d1 = d1 + e1 + $[PERMUTE_BOTH[z++]] + (d0 / Mod.U | 0) | 0,
    d0 >>>= 0,
    q = o0 ^ d0,
    j0 += o0 = (q >>> 16 ^ o1 << 16) >>> 0,
    o1 = (o1 >>> 16 ^ q << 16) >>> 0,
