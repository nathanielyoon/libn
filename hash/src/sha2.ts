import { iv, min, SHA256, SHA512 } from "./common.ts";

const HI = /* @__PURE__ */ iv(
...
type Hash = ($: Uint8Array) => Uint8Array<ArrayBuffer>;
const md = (
  base: Uint32Array,
...
($) => {
  ...
  const d = new Uint32Array(b * 10), e = new Uint8Array(c), f = $.length;
  let g = new DataView($.buffer, $.byteOffset), z = 0, y = 0;
  while (z < f) {
    const j = min(c - y, f - z);
    if (j !== c) e.set($.subarray(z, z += j)), y += j;
...
/** Hashes with SHA-256. */
export const sha256: Hash = /* @__PURE__ */ md(SHA256, (use, data, at, to) => {
  let a, b, z = 0;
...
/** Hashes with SHA-512. */
export const sha512: Hash = /* @__PURE__ */ md(SHA512, (use, data, at, to) => {
  let a, b, c, d, z = 0;
  do use[z] = data.getUint32(at), use[z + 80] = data.getUint32(at + 4); while (
    at += 8, ++z < 16
  );
  ...
  do a = (n >>> 9 | m << 23) ^ (m >>> 14 | n << 18) ^ (m >>> 18 | n << 14),
    ...
    u = t + (b >>> 0) + ((n & p ^ ~n & r) >>> 0) + LO[z] + use[z + 80],
    v = a + s + (m & o ^ ~m & q) + HI[z] + use[z] + (u / 0x100000000 | 0) | 0,
