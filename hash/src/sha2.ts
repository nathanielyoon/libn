import { en_iv, type Hash, min, Mod, SHA256, SHA512 } from "./common.ts";

type Mix = (block: DataView, at: number, $: Uint32Array) => void;
const md = (length: number, iv: Uint32Array, mix: Mix, $: Uint8Array) => {
  const size = iv.length << 3, max = $.length, block = new Uint8Array(size);
...
const mix_80: Mix = (block, at, $) => {
  ...
  do a = L80[z - 15],
    ...
    L80[z] = d + ((a >>> 19 | b << 13) ^ (b >>> 29 | a << 3) ^ a >>> 6) +
      L80[z - 7] + L80[z - 16] + (c / Mod.U | 0); while (++z < 80);
  ...
  do a = (n >>> 9 | m << 23) ^ (m >>> 14 | n << 18) ^ (m >>> 18 | n << 14),
    ...
    v = a + s + (m & o ^ ~m & q) + K80[z] + L80[z] + (u / Mod.U | 0) | 0,
