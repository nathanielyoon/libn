...
const md = (base: Uint32Array, mix: Mix, length: number, $: Uint8Array) => {
...
const mix_64: Mix = (block, at, $) => {
  let a, b, z = 0;
  do WORDS[z] = block.getUint32(at), at += 4; while (++z < 16);
  do a = WORDS[z - 2],
    b = WORDS[z - 15],
    WORDS[z] = ((b >>> 7 | b << 25) ^ (b >>> 18 | b << 14) ^ b >>> 3) +
      ((a >>> 17 | a << 15) ^ (a >>> 19 | a << 13) ^ a >>> 10) +
      WORDS[z - 7] + WORDS[z - 16]; while (++z < 64);
  let c = $[z = 0], d = $[1], e = $[2], f = $[3], g = $[4], h = $[5], i = $[6];
  let j = $[7];
  do a = ((g >>> 6 | g << 26) ^ (g >>> 11 | g << 21) ^ (g >>> 25 | g << 7)) +
    (g & h ^ ~g & i) + j + KL[z] + WORDS[z],
...
const UPPER = new Uint32Array(80), LOWER = new Uint32Array(80);
const mix_80: Mix = (block, at, $) => {
  let a, b, c, d, z = 0;
  do LOWER[z] = block.getUint32(at),
    UPPER[z] = block.getUint32(at + 4),
    at += 8; while (++z < 16);
  do a = LOWER[z - 15],
    b = UPPER[z - 15],
    ...
    a = LOWER[z - 2],
    b = UPPER[z - 2],
    UPPER[z] = c += UPPER[z - 16] + UPPER[z - 7] + (((b >>> 19 | a << 13) ^
      (a >>> 29 | b << 3) ^ (b >>> 6 | a << 26)) >>> 0),
  ...
  do a = (n >>> 9 | m << 23) ^ (m >>> 14 | n << 18) ^ (m >>> 18 | n << 14),
    ...
    u = t + (b >>> 0) + ((n & p ^ ~n & r) >>> 0) + KH[z] + UPPER[z],
    v = a + s + (m & o ^ ~m & q) + KL[z] + LOWER[z] + (u / Mod.U | 0) | 0,
